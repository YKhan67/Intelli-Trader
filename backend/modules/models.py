...
class RiskParameters(BaseModel):
    ...
    lot_size: float = Field(..., gt=0, description="Calculated lot size for the trade")
    stop_loss_price: float = Field(..., gt=0, description="Price level for stop loss")
    take_profit_price: float = Field(..., gt=0, description="Price level for take profit")
...
class BackendSignal(BaseModel):
    ...
    regime_result: MarketRegimeResult = Field(..., description="Contextual regime data")
    sentiment_result: SentimentResult = Field(..., description="Contextual sentiment data")
    risk_params: RiskParameters = Field(..., description="Contextual risk data")
